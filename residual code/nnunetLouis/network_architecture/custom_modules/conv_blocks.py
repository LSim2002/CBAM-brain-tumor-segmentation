...
class DeconvConvDropoutNormReLU(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, network_props):
        """
        if network_props['dropout_op'] is None then no dropout
        if network_props['norm_op'] is None then no norm
        ...
        :param network_props:
        """
        super(DeconvConvDropoutNormReLU, self).__init__()

        network_props = deepcopy(network_props)  # network_props is a dict and mutable, so we deepcopy to be safe.
        
        self.deconv = network_props['deconv_op'](input_channels, output_channels, kernel_size - 1,
                                             padding=[(i - 1) // 2 for i in kernel_size - 1],
                                             **network_props['deconv_op_kwargs'])

        self.conv = network_props['conv_op'](output_channels, output_channels, kernel_size,
                                             padding=[(i - 1) // 2 for i in kernel_size],
                                             **network_props['conv_op_kwargs'])
        ...
        self.all = nn.Sequential(self.deconv, self.conv, self.do, self.norm, self.nonlin)

    def forward(self, x):
        return self.all(x)

class DecodeurLayer(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, network_props):
        """
        if network_props['dropout_op'] is None then no dropout
        if network_props['norm_op'] is None then no norm
        ...
        self.all = nn.Sequential(
            ConvDropoutNormReLU(output_channels*2, output_channels, kernel_size, network_props),
...
#
#       Class: FullpreactivationResidualBlock
#       Author: Eni Gjineci
#
class FullpreactivationResidualBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        """
        This is the | bn | nonlin | conv | bn | nonlin | conv | kind of block
        :param in_planes:
        :param out_planes:
        :param props:
        :param override_stride:
    ...
    def forward(self, x):
        ...
        out = self.conv1(out)
        out = self.nonlin2(self.norm2(out))
        out = self.conv2(out)

        residual = self.downsample_skip(residual) #if hasattr(self, 'residual') else x
        out += residual
...
class PreactivationResidualBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        """
        This is the | nonlin | conv | bn | nonlin | conv | bn | kind of block
        :param in_planes:
        :param out_planes:
        :param props:
        :param override_stride:
    ...
    def forward(self, x):
        ...
        out = self.conv1(self.nonlin1(x)) 
        out = self.norm1(out)
        out = self.conv2(self.nonlin2(out))
        out = self.norm2(out)
...
#
#       Class: NormAfterAdditionResidualBlock
#       Author: Nicolas Rougon
#
class NormAfterAdditionResidualBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        """
        This is the | conv | bn | nonlin | conv | bn | nonlin | kind of block
        :param in_planes:
...
#
#       Class: NonlinBeforeAdditionResidualBlock
#       Author: Nicolas Rougon
#
class NonlinBeforeAdditionResidualBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        """
        This is the | conv | bn | nonlin | conv | bn | nonlin | kind of block
        :param in_planes:
