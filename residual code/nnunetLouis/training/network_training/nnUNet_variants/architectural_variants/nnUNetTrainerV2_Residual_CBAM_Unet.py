...
from nnunet.network_architecture.custom_modules.conv_blocks import FullpreactivationResidualBlock

from nnunet.network_architecture.generic_modular_residual_cbam_UNet import  FabiansCBAMUNet, get_default_network_cbam_config
...
class nnUNetTrainerV2_ResidualCbamUNet(nnUNetTrainerV2):
    def initialize_network(self):
        if self.threeD:
            cfg = get_default_network_config(3, None, norm_type="in")
            cbam_props = get_default_network_cbam_config(dim=3)

        else:
            cfg = get_default_network_config(1, None, norm_type="in")
            cbam_props = get_default_network_cbam_config(dim=2)
        ...
        self.network = FabiansCBAMUNet(self.num_input_channels, self.base_num_features, blocks_per_stage_encoder, 2,
                                   pool_op_kernel_sizes, conv_kernel_sizes, cfg, cbam_props, self.num_classes,
                                   blocks_per_stage_decoder, True, False, 320, InitWeights_He(1e-2), FullpreactivationResidualBlock)
